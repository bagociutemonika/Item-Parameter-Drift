...
# Simulation 2.1 — XGBoost DIF Detection
# Rasch Calibration
...
xgb_summary_rasch_2_1 <- data.frame()
...
for(i in 1:length(datasets_2PL)){
  ...
  for(it in unique(df_ml$item_id)){
    ...
    for(k in 1:K){
      ...
      # Model 0: ability only (1 column)
      dtrain0 <- xgb.DMatrix(matrix(train_data$theta_hat, ncol = 1),
                             label = train_data$response)
      dtest0  <- xgb.DMatrix(matrix(test_data$theta_hat, ncol = 1))
      ...
      # Model 1: ability + group (2 columns)
      dtrain1 <- xgb.DMatrix(cbind(train_data$theta_hat, train_data$group_bin),
                             label = train_data$response)
  ...
  xgb_summary_rasch_2_1 <- rbind(xgb_summary_rasch_2_1, data.frame(
...
xgb_summary_rasch_2_1 <- xgb_summary_rasch_2_1 %>%
  mutate(
