...
# Simulation 1.2 — XGBoost DIF Detection
...
for(i in 1:length(datasets)){
  ...
  # --- Long-format with group_bin for XGBoost (domain removed) ---
  df_ml <- data_wide %>%
  ...
  for(it in unique(df_ml$item_id)){
    ...
    for(k in 1:K){
      ...
      # Model 0: theta_hat only, no group, no domain
      ...
      # Model 1: theta_hat + group_bin, no domain
  ...
  xgb_summary_1_2 <- rbind(xgb_summary_1_2, data.frame(
...
xgb_summary_1_2 <- xgb_summary_1_2 %>%
  mutate(
